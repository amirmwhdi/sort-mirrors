...
A powerful, lightweight, and interactive Bash script designed to automatically find, test, and apply the fastest package mirrors for **Arch Linux**, **Ubuntu**, and **Debian** based on your network's real-time latency.

Tired of slow package downloads? This script fetches official mirror lists, tests their response times concurrently, and safely applies the fastest one to your system.

## ✨ Features

- **Auto-Detection:** Automatically detects your Linux distribution and version (codename).
...
- **Broad Compatibility:**
    ...
    - Auto-detects CPU architecture (`x86_64`, `aarch64`, etc.) for Arch Linux.
- **Safe & Secure:** Always creates a `.backup` of your original mirror configuration files before making any changes.
- **Robust Fallbacks:** If HTML parsing of mirror lists fails, the script automatically falls back to raw text mirror lists to ensure it never fails.
...
2. **Fetch:** Downloads the official mirror list from the respective distribution's website.
3. **Parse:** Extracts valid HTTP/HTTPS URLs using `grep` and `sed` (standardizing trailing slashes).
4. **Test:** Uses `curl` to download a tiny test file (like `Release` or `core.db`) from each mirror, measuring the `time_total`.
